/*
 * dual_30g_fullscan.cu — dual-process 30G each, full GPU byte verify.
 * Usage: dual_30g_fullscan <hexpat> <left|right>
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int fullscan(void *dev, unsigned char pat, const char *tag) {
    ...
    CHK(cudaFree(d_bad));
    printf("%s FULLSCAN bad=%llu / %llu %s\n",
           tag, h_bad, (unsigned long long)SZ, h_bad ? "FAIL" : "OK");
    fflush(stdout);
    return h_bad ? 1 : 0;
}
...
static int band_samples(void *dev, unsigned char pat, const char *tag) {
    ...
    int fail = 0;
    static const size_t offs_g[] = {0, 1, 4, 8, 16, 24, 29};
...
static int touch_write(void *dev, unsigned char pat, const char *tag) {
    CHK(cudaMemset(dev, pat, 4 * 1024 * 1024));
    CHK(cudaDeviceSynchronize());
    printf("%s TOUCH wrote 4MiB @0 pat=0x%02X\n", tag, pat);
    fflush(stdout);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    char myflag[128], peer_left[128], peer_right[128];
    snprintf(myflag, sizeof(myflag), "/tmp/dual30g_ready_%s", side);
    snprintf(peer_left, sizeof(peer_left), "/tmp/dual30g_ready_left");
    snprintf(peer_right, sizeof(peer_right), "/tmp/dual30g_ready_right");
    ...
    printf("%s pid=%d ptr=%p pat=0x%02X size=30G\n", side, getpid(), dev, pat);

    printf("%s phase=SOLO-FILL-VERIFY\n", side);
    ...
    for (int i = 0; i < 180; i++) {
        if (access(peer_left, F_OK) == 0 && access(peer_right, F_OK) == 0) break;
        usleep(200000);
    }
    sleep(2);

    printf("%s phase=PEER-UP-VERIFY\n", side);
    int fail = band_samples(dev, pat, side) | fullscan(dev, pat, side);

    fail |= touch_write(dev, pat, side);
    fail |= band_samples(dev, pat, side);

    printf("%s phase=PEER-UP-AFTER-TOUCH\n", side);
    fail |= fullscan(dev, pat, side);
