...
| **v63-fix-40gb-profile** | **紧急修复：`install.sh --profile=10gb`（从 80G 切 40G）冷启动 wedge(GSP 不起、nvidia-smi 卡死）。dmesg 证据链：首 attempt WPR meta fbSize=0xa00000000(40G,fb_length 已按 10gb 生效）→ PCIe PRIV_MISC_1 booter attempt=1 status=0x65 FAILED → normal BooterLoad 0x65/mailbox 0x31;retry 时 `CMP_MEM_EARLY_WRITE: cfg1 before=0x02669000 target=0x02779000 after=0x02779000 lmr before=0x0000028a target=0x0000028b`——正确的 40G 值被 early-write 强改 80G → 首个 populate 即 80G meta，与编译期 40G 常量同模块两套几何 → wedge。根因：profile 传递链（install.sh → build.sh → apply_profile.py）只重写 sec2 POST-WRITE 块（cfg1Value/lmrValue）和 targetFbBytes;`early-lmr-write-p1a.patch` 的 P1a early-write 块（cfg1Target/lmrTarget,2082 分支）硬编码 80G(0x02779000/0x0000028B)，从没跟着 profile 走。修复：(1) P1a patch 出厂默认改为稳定 40G 值（fail-safe，与 sec2 patch 惯例一致）,hunk 头行数同步修正；(2) apply_profile.py 新增 EARLY_WRITE_BLOCK_RE，按 profile 同步重写 P1a 块并 fail-closed 校验（marker=CMP_MEM_EARLY_WRITE，缺块/多块都报错）;(3) phantom reserve 实查不符"80G only"标注（代码只 gate devId==0x2082):40G profile 下 [36G,41G) pin 会吃掉堆顶 4G 且 MIG zero-check 白加 5G 容忍——改为 build.sh 传 `--profile`，仅 10gb64/10gb80/mixed80（堆越过 40G+64K 幻影结构）应用，8gb/10gb/mixed 整体跳过（pin + MIG 容忍都不注入）;(4) apply_tail_steer_pin.py 锚点全在 phantom 插入文本上且 corridor 尾址 0x13f410ffff 是 80G 布局地址，同步加 `--profile` 门控（否则 10gb 构建在 pin anchor 处 FAIL——第一次重建实测踩中）;(5) dev helper apply_early_lmr_p1a.py 同步到 40G 默认值。Booter 0x65 定性：混合几何的直接下游症状——P1a 在 PLM 循环前把 LMR 写成 80G,booter 循环按 80G latch,POST-WRITE 翻回 40G 后所有后续 BooterLoad（含 PRIV_MISC_1 的 payload 运送）全部 0x65；不是 PCIe 写的独立故障（status=0x65 是 kgspExecuteBooterLoad 返回值，同一代码路径在 10gb80 生产 build 里 Gen2 工作正常）。几何一致后 PRIV_MISC_1 路径无需额外担心** | **修复后生产构建：`CMPUNLOCKER_PRODUCTION=1 CMPUNLOCKER_STRIP_POST0808=1 CMPUNLOCKER_DRIVER_VERSION=610.43.02 CMPUNLOCKER_CARD_PROFILE=10gb ./build.sh`。本地 10/10 unittest 绿（含 P1a patch 出厂默认值/双 profile 重写/无 marker 跳过/坏块 fail-closed 新用例）;patch 干跑应用干净；服务器 test_patch_application 3/3 绿（hunk 行数校验）。构建期自检（服务器实测）：10gb 制备树 kernel_gsp.c 无 0x0000028B(0 次）;0x0000001400000000 仅剩 1 处在 apply_tail_steer.py 注入的 `targetFbBytes == 80G && RMCmpTailSteer!=0` 运行期条件里（默认关，40G 下死分支，非写入路径）;mem_mgr.c 无 CMP_MEM_RSV/CMP_TAIL_PIN/cmpPhantomRsv；安装态 nvidia.ko 反汇编：imm 0x2669000×2(P1a+POST-WRITE 两个 2082 分支）、0x28a×10、0xa00000000×1、0x2779000×2（两个 20c2 8GB 卡分支，合法）,0x1400000000 立即数 0 次，$0x28b 的 13 处全为无关常量（源树无任何 0x28B 字面量，651 是常见 NV2080/错误码值）——80G 目标值不再有任何无条件写入路径。10gb80 路径回归验证：apply_profile 重写后 P1a/sec2/fb_length 三块与修复前逐值一致，phantom+tail pin 照常注入。**模块已装（未激活，hot-reload 默认关），机器保持 wedge 现场未动（不 reboot/不 FLR），待用户冷启动验证** |
| **v64-acceptance-40gb** | **v63 修复后用户冷启动，40G profile 快速验收（对照 v62-acceptance 的 80G 基线）** | **关键项全绿，验收由用户提前终止：(1) nvidia-smi:40960 MiB / 610.43.02 / Firmware 610.43.02 / Persistence Off / x4。(2) dmesg POST-WRITE:SS0=0x88888888 SS1=0x00000008 CFG1=0x02669000 LMR=0x0000028a——与 v62 生产值逐值一致；CMP_MEM_EARLY_WRITE target 已是 0x02669000/0x28a(v63 修复生效的直接实证）,WPR meta fbSize=0xa00000000 单一几何。(3) Gen2:pcie.link.gen.current=2。(4) **用户自跑 gpu_burn -m 95% 1800(09:02-09:33):100% 完成、GPU 0: OK、errors=0（输出经 strace 截获 stdout 恢复——裸 pts 无日志；27.7% 起累计 errors=0 ⇒ 首窗 10.3% = 0，与 80G 配置首窗 45539328 错误形成对照——"首窗报错=折叠墙脚印"判读在 40G 下反向验证：无墙）**,11685 Gflop/s 持续，峰值 85C。(5) bench_matmul(gpuenv/torch 2.13.0+cu126):BF16 181.28 / FP16 178.13 / FP32 12.27 / FP64 11.67 TFLOPS——与 v62 同档（BF16/FP16 略超 166.5/168.9,FP32 持平 12.26,FP64 -3% 噪声内），满血无节流（nerfed 基线 6.15/6.26/0.39/0.20)。(6) 复跑方 gpu_burn -m 85% 300 与 llama 256K 未执行：09:36:36 用户主动 poweroff(journal 为有序关机，全程零 Xid，非 hang/crash),09:39:26 再开机时卡已变为 10de:20c2(8G SKU)——驱动按 PCI ID 正确走 64GB 分支（POST-WRITE CFG1=0x02779000/LMR=0x20b,65536 MiB，零 Xid,mixed 兼容路径实证）。机器留在用户当前生产态（20c2/64GB，用户已有负载在跑）。40G 配置判词：v63 修复后冷启动可用、算力满血、Gen2 在、30 分钟 95% 极限烧机零错误** |
